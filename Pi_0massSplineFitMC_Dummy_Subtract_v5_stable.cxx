...
int main(int argc,char* argv[])
{
    ...
    int dbgPrinted = 0;
    auto dbgOk = [&](bool force=false){ return DEBUG_MM && (force || dbgPrinted<DEBUG_MAX); };
    auto dbgBump = [&](){ ++dbgPrinted; };
    ...
    { TFile outHist("charge_normalized_raw_yield.root", "RECREATE"); hAll.Write(); }
    ...
    hSig.SetBins(nBins, sigLo, sigHi); hSig.SetDirectory(nullptr);
    ...
    if(hDumNorm){
        hSub_before = hSub;
        hD = *hDumNorm; hD.Scale(realQ); // back to raw
        hSub.Add(&hD,-1.0);
        hSub_after = hSub;
    }
    ...
    for (Long64_t ie = 0; ie < Nr; ++ie) {
        ...
        // ---- DEBUG PRINTS ----
        bool bad = (!(std::isfinite(peprime_px)&&std::isfinite(peprime_py)&&std::isfinite(peprime_pz)&&std::isfinite(Ep)))
                   || (r1<=0 || r2<=0)
        ...
        for (size_t a = 0; a < keep.size(); ++a)
            for (size_t b = a + 1; b < keep.size(); ++b) {
                ...
                double xra,yra,zra; rotateNPS(cX[j1], cY[j1], xra, yra, zra, zDet_evt);
                double xrb,yrb,zrb; rotateNPS(cX[j2], cY[j2], xrb, yrb, zrb, zDet_evt);
    ...
    { double maxMM = hMissMass.GetMaximum();
      if (maxMM > 0) hMissMass.SetAxisRange(0, maxMM * 1.2, "Y"); }
    hMissMass.Draw("HIST");
