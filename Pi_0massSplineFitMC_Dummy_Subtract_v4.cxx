    ...
    *************************************************************/
    ...
    // rotation about +y by NPS_theta_rad; zDet defaults to nominal NPS_dist
        inline void rotateNPS(double xDet, double yDet,
                            double &xBeam, double &yBeam, double &zBeam,
                            double zDet = NPS_dist)
        {
            xBeam =  xDet * std::cos(NPS_theta_rad) + zDet * std::sin(NPS_theta_rad);
            yBeam =  yDet;
            zBeam = -xDet * std::sin(NPS_theta_rad) + zDet * std::cos(NPS_theta_rad);
        }
    ...
    int main(int argc,char* argv[])
    {
       ...
       /* ------------------------------------------------------------------
        Event loop – NPS rotation applied once per cluster
        ------------------------------------------------------------------ */
       const double DNPS = 407.0;                    // cm
        struct Cl { int tbin; double m; };
        struct Ev { int tbin; double q2; };

        std::vector<Cl> cls;     cls.reserve(1'000'000);
        std::vector<Ev> evtList; evtList.reserve(500'000);

        for (Long64_t ie = 0; ie < Nr; ++ie) {
            tr->GetEntry(ie);
            if (!passHMSCuts(edt, dp, et, npe, th, ph)) continue;

            /* event-by-event Q² using HMS slopes */
            ...
            /* Select good clusters with DETECTOR-frame cuts */
            std::vector<int> keep; keep.reserve((int)ncl);
            for (int c = 0; c < (int)ncl; ++c) {
                if (cT[c] < sigLo || cT[c] > sigHi) continue;
                if (!isGoodCluster(cE[c], cT[c], cX[c], cY[c])) continue;  // detector x/y
                keep.push_back(c);
            }
            if (keep.size() < 2) continue;


            // >>> Compute event-specific detector Z **here** <<<
            double zDet_evt = NPS_dist;
            /* ---------- Missing-mass calculation with rotation ---------- */
            int i1 = keep[0], i2 = keep[1];
            /* Photon 1: rotate (x,y) to beam frame, then use the rotated z */
            double E1 = cE[i1];
            double xr1, yr1, zr1;
            rotateNPS(cX[i1], cY[i1], zDet_evt, xr1, yr1, zr1);
            double r1  = std::sqrt(xr1*xr1 + yr1*yr1 + zr1*zr1);
            double p1x = E1 * xr1 / r1;
            double p1y = E1 * yr1 / r1;
            double p1z = E1 * zr1 / r1;

            /* Photon 2 */
            double E2 = cE[i2];
            double xr2, yr2, zr2;
            rotateNPS(cX[i2], cY[i2], zDet_evt, xr2, yr2, zr2);
            double r2  = std::sqrt(xr2*xr2 + yr2*yr2 + zr2*zr2);
            double p2x = E2 * xr2 / r2;
            double p2y = E2 * yr2 / r2;
            double p2z = E2 * zr2 / r2;


            /* Scattered-electron 4-vector (unchanged) */
            const double am = 0.938;                // proton mass
            double e0 = e0_nom;
            double ep = ep0_nom * (1.0 + dp / 100.0);
            double peprime_p  = std::sqrt(ep*ep - 0.000511*0.000511);
            double peprime_px = peprime_p * ph;
            double peprime_py = peprime_p * th;
            double peprime_pz = peprime_p * (1.0 - 0.5*(th*th + ph*ph));
            ...
            double E_in  = e0 + am;
            double px_in = 0.0, py_in = 0.0, pz_in = e0;
            ...
            /* ---------- π0 candidate list for Toy-MC ---------- */
            for (size_t a = 0; a < keep.size(); ++a)
                for (size_t b = a + 1; b < keep.size(); ++b) {
                 int j1 = keep[a], j2 = keep[b];

                // rotate each cluster to beam frame
                double xra,yra,zra; rotateNPS(cX[j1], cY[j1], zDet_evt, xra, yra, zra);
                double xrb,yrb,zrb; rotateNPS(cX[j2], cY[j2], zDet_evt, xrb, yrb, zrb);
                ...
                cls.push_back({tb, m});   // store cluster mass and time bin
                }
        }
